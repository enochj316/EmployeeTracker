...
let employeeInquiry = (cli, connection, roles, employees) => {
...
let getEmployees = (cli, roles, employee, connection) => {
    let employees = [];
    connection.query(`
  SELECT id, first_name, last_name, CONCAT(first_name, ' ', last_name) AS full_name FROM employee;;`,
        (err, res) => {
            ...
            employeeInquiry(cli, roles, employees, connection);
            connection.end();
        });
}

// Queries than passes role info
let getRoles = (cli, connection) => {
    ...
    connection.query(`SELECT * from role;`,
        ...
        (err, res) => {
            ...
            getEmployees(cli, roles, employee, connection);
            connection.end();
        });
}
